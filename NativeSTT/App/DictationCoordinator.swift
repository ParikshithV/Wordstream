...
final class DictationCoordinator {
    ...
    private let log = Logger(subsystem: "app.nativestt", category: "dictation")
    ...
    /// Loads the selected model, or the per-device recommendation when nothing
    /// has been chosen yet.
    func prepareModel() async {
        await engine.refreshCatalogue()
        let variant = prefs.modelVariant.isEmpty
            ? (engine.recommendedModel ?? "")
            : prefs.modelVariant
        guard !variant.isEmpty else { return }
        if prefs.modelVariant.isEmpty { prefs.modelVariant = variant }
    ...
    private func finish() {
        ...
        guard duration >= 0.35, hasSpeech(samples) else {
            ...
            log.info("Discarded \(String(format: "%.2f", duration))s of audio with no speech.")
            reset()
            return
        }
